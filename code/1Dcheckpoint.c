...
void load_checkpoint(double *T, int n, int m, int o, double *t, int *n_steps, int rank) {
    ...
    if (checkpoint_file == NULL) {
    fprintf(stderr, "2 : normalement pas de print au 2e appel \n");
    ...
    fprintf(stderr, "5 : scanf reussi du tableau \n");
...
int main()
{
    ...
    double o_double = (double)o; // Conversion de o en double
    ...
    if(rank< nbr_processk){
        ...
        load_checkpoint(T, n, m, ot, &t, &n_steps, rank);
        ...
        while (convergence == 0) {
            ...
            if(rank==0){
                MPI_Sendrecv(T+(kend_pl-kstart_pl -1 + 1)*n*m , n*m, MPI_DOUBLE, 1, 0, T+(1+kend_pl-kstart_pl)*n*m , n*m, MPI_DOUBLE, 1, 0, subset_comm, MPI_STATUS_IGNORE);
            }
            else if((rank>0)&(rank<size-1)){
                MPI_Sendrecv(T +n*m , n*m, MPI_DOUBLE, rank-1, 0, T, n*m, MPI_DOUBLE, rank-1, 0, subset_comm, MPI_STATUS_IGNORE);
                MPI_Sendrecv(T+(kend_pl-kstart_pl)*n*m , n*m, MPI_DOUBLE, rank+1, 0, T+(kend_pl-kstart_pl+1)*n*m, n*m, MPI_DOUBLE, rank+1, 0, subset_comm, MPI_STATUS_IGNORE);   
            }
            else{
                MPI_Sendrecv(T +n*m  , n*m, MPI_DOUBLE, rank-1, 0, T, n*m, MPI_DOUBLE, rank-1, 0, subset_comm, MPI_STATUS_IGNORE);
            }
