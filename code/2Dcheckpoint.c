...
// function to save the state to a checkpoint file for each process
void save_checkpoint(const double *T, int n, int m, int o, double t, int n_steps, int rank) {
    char filename[40];
    sprintf(filename, "checkpoint_rank%d.txt", rank);
    ...
    fprintf(checkpoint_file, "%d %d %d %lf %d\n", n, m, o, t, n_steps);
    for (int u = 0; u < n * m * o; u++) {
        fprintf(checkpoint_file, "%lf ", T[u]);
    }

    fclose(checkpoint_file);
}

// function to load the state from a checkpoint file for each process
void load_checkpoint(double *T, int n, int m, int o, double *t, int *n_steps, int rank) {
    char filename[40];
    sprintf(filename, "checkpoint_rank%d.txt", rank);
    ...
    fscanf(checkpoint_file, "%d %d %d %lf %d", &n, &m, &o, t, n_steps);
    //fprintf(stderr, "4 : scanf reussi des var \n");

    for (int u = 0; u < n * m * o; u++) {
        fscanf(checkpoint_file, "%lf", &T[u]);
    }
...
int main()
{
    ...
    if(rank< nbr_processk*nbr_processy){
        ...
        int ot = (kend_pl-kstart_pl + 2);
        int mt = (yend_pl-ystart_pl+2);
        ...
        load_checkpoint(T, n, mt, ot, &t, &n_steps, rank);
        ...
        while (convergence == 0) {
            ...
            if (rank % (petit_diviseur)==0){
                ...
                for(int lp = kstart_pl;lp<kend_pl;lp++){
                    for(int p=0;p<n;p++){
                        T_cote1[p+n*(lp-kstart_pl)] = T[ n + n*(yend_pl-ystart_pl+2)*(lp-kstart_pl+1)  + p ] ; // T_del + p + n + n*(yend-yst)(l-kstart_pl)
                    }
                }
                ...
                for(int lp = kstart_pl;lp<kend_pl;lp++){
                    for(int p=0;p<n;p++){
                        T_cote1[p+n*(lp-kstart_pl)] = T[ p + n + n*(yend_pl-ystart_pl+2)*(lp-kstart_pl+1) ] ; // T_del + p + n + n*(yend-yst)(l-kstart_pl)
                    }
                }
            ...
            if (n_steps % CHECKPOINT_INTERVAL == 0) {
                save_checkpoint(T, n, m, ot, t, n_steps, rank);
            }
        ...
        if(rank ==0){
            ...
            /// connaitre processes 
            int proce_state1 = (int)k/kpl_per_process; // process entre < (proce_state+1)processy &&  >= (proce_state)processy
