...
int main()
{
    ...
    ////////////////////////////////////////////////////////
    /* Determiner le nbr de process dont on a besoin il peut y avoir des process qu'on utilise pas */
...
//    int nbr_processy = ceil(m/ypl_per_process) ;
    ...
    if(rank< nbr_processk*nbr_processy){
        ...
        while (convergence == 0) {
            ...
            if (rank % (petit_diviseur)==0){
                ...
                MPI_Isend(T_cote2 , n*(kend_pl-kstart_pl), MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, &request);
                MPI_Irecv( T_cote1, n*(kend_pl-kstart_pl), MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, &request2);
                ...
                MPI_Isend(T_cote1 , n*(kend_pl-kstart_pl), MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD, &request);
                MPI_Irecv(T_cote2, n*(kend_pl-kstart_pl), MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD, &request2);
                ...
                MPI_Isend(T_cote1 , n*(kend_pl-kstart_pl), MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, &request3);
                MPI_Irecv(T_cote2, n*(kend_pl-kstart_pl), MPI_DOUBLE, rank+1, 0, MPI_COMM_WORLD, &request4);
                ...
                for(int lp = kstart_pl;lp<kend_pl;lp++){
                    for(int p=0;p<n;p++){
                        T[ n*(yend_pl-ystart_pl+1) + n*(yend_pl-ystart_pl+2)*(lp-kstart_pl+1)+p   ]   =  T_cote2[p+n*(lp-kstart_pl)]   ;        
                    }
                }
            ...
            if(rank<petit_diviseur){
                ...
                MPI_Isend(T  + (kend_pl-kstart_pl -1 + 1)*n*(yend_pl-ystart_pl+2) , n*(yend_pl-ystart_pl+2), MPI_DOUBLE, rank+(petit_diviseur), 0, MPI_COMM_WORLD, &request3);
                MPI_Irecv(T + (kend_pl-kstart_pl + 1)*n*(yend_pl-ystart_pl+2), n*(yend_pl-ystart_pl+2), MPI_DOUBLE, rank+(petit_diviseur), 0, MPI_COMM_WORLD, &request4);
                ...
                MPI_Isend(T  + n*(yend_pl-ystart_pl+2), n*(yend_pl-ystart_pl+2), MPI_DOUBLE, rank-(petit_diviseur), 0, MPI_COMM_WORLD, &request);
                MPI_Irecv(T , n*(yend_pl-ystart_pl+2), MPI_DOUBLE, rank-(petit_diviseur), 0, MPI_COMM_WORLD, &request2);
                ...
                MPI_Isend(T  +(kend_pl-kstart_pl)*n*(yend_pl-ystart_pl+2) , n*(yend_pl-ystart_pl+2), MPI_DOUBLE, rank+(petit_diviseur), 0, MPI_COMM_WORLD, &request3);
                MPI_Irecv(T  +(kend_pl-kstart_pl+1)*n*(yend_pl-ystart_pl+2), n*(yend_pl-ystart_pl+2), MPI_DOUBLE, rank+(petit_diviseur), 0, MPI_COMM_WORLD, &request4);
            ...
            for (int k = kstart_pl; k < kend_pl ; k++) {   // z 
                ...
                for (int j = ystart_pl; j < yend_pl; j++) {   // y
                    for (int i = 0; i < n; i++) {   // x
                        ...
                        int u =  (k-kstart_pl+1) * n * (yend_pl-ystart_pl+2) + (j-ystart_pl+1) * n + i;    // int u = (k-kstart_pl+1) * n * (yend_pl-ystart_pl+2) + (j-ystart_pl+1) * n + i;
            ...
            if (n_steps % ((int)(1 / dt)) == 0) {
                ...
                MPI_Allreduce(&delta_T, &sum , 1, MPI_DOUBLE, MPI_SUM, subset_comm); 
